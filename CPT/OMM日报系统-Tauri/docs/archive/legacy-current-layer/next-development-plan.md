...
- 下次每件时间默认建议使用 `2~5 分钟/件`。
- 普通测料当前普遍偏多，最低可接受时间按 `1 分钟/件` 校验，不应只因件数多就判定偏短。
- CNC 任务最低按 `20 分钟/任务` 计入；若按件数计算低于该值，应抬到 20 分钟。
- 如果当日日报时间不够，推荐策略倾向补足日报缺口：根据当天最低需要时间、已确认时间和普通任务 PCS 反推建议每件时间。
- 弹窗不要只提示“偏短”，应给出建议范围、依据和操作入口，例如“套用建议并继续 / 打开单日设置 / 先不继续”。
- 如果反推出来的每件时间过高，不要硬套用，应建议补录手量或其他事项。
- 手量补录/生成展示中，手量项目后面要明确写 `手量`，避免报表里看不出该项来源。
- 送测日期格式不要写 `6月28日` 这类中文日期；日期本体统一写 `6.28`，班次只保留 `A`、`B` 或空值。
