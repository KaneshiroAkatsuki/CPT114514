...
### 1.18 gpt 本轮：文件名疑似错误/需确认规则精确化

按用户要求，将“文件名比较疑惑或者觉得错误”从主观感觉改成可解释规则。识别结果不自动强行纠错，而是在手量候选卡片、粘贴识别日志、识别补充测试窗口里显示“需确认”原因；保存/生成仍需要用户人工确认手量字段。

- **疑似错误/需确认触发规则**
  1. **品名异常**：未识别到品名；品名不像三位数字、`测试片`、`0.2/0.25` 或烧结盘多品名；品名像人名或机器/模板名。
  2. **多候选歧义**：普通任务中识别到多个三位数字品名，或多个 `PCS/件` 数量；程序只取第一个并提示确认。
  3. **送测人疑似错字**：出现 `张三送`、`张三送检`、`张三送样`、`张三生成`、`张三上传`、`张三提交` 等，先按 `张三` 预填送测人，并提示用户确认是否为 `张三送测`。
  4. **送测时间异常**：类似 `25点70` 这种不在 `0:00-23:59` 范围内的时间会提示确认。
  5. **手量测量员异常**：出现 `手量/手测` 但没有 `-手量-姓名`，或 `-手量-送` 这类不像 2-4 个中文字姓名的写法。
  6. **工站兜底**：未识别明确工站时，按 `开发` 兜底，但提示用户确认。
  7. **CMM/OMM 后姓名**：识别逻辑不会把 `CMM/OMM` 后的人名当作品名或手量测量员；该情况属于正常忽略，不再打扰式 warning。

- **前端显示**
  - `ManualTaskDialog` 的自动发现候选卡片会显示 `recognition_warnings`。
  - 粘贴文件夹名识别后，日志会显示“需确认：...”。
  - 非候选来源的手量记录也会显示识别 warning。
  - `RecognitionRulesDialog` 已有测试识别区，会继续显示这些提示。

- **代码级冒烟用例**
  - 正常：`565-开发-MO-T0模具测试-安容克送测-96PCS-CMM-郑安午-手量-禹欣` → 无 warning。
  - 错字：`安容克送` / `安容克送检` / `安容克上传` → 预填送测人 `安容克`，并提示确认是否为 `安容克送测`。
  - 正常测试片：`测试片-FAI-刘前程2点30送测-手量-王业陈` → 无 warning。
  - 异常时间：`刘前程25点70送测` → 提示时间异常。
  - 多数量/测量员异常：`96PCS-12PCS-...-手量-送` → 提示多个数量、测量员不像姓名。
  - 缺品名：`EVT-SLIDER-开发-安容克送测-手量-禹欣` → 提示未识别到品名。
  - 送测错字：`安容克上传` → 提示确认是否为 `安容克送测`。

- **帮助文档同步**
  - `HelpCenterDialog` 的命名规则章节新增“哪些文件名会提示人工确认”。

- **影响范围**
  - 只改前端识别提示、手量弹窗展示和帮助文档。
  - 未触碰 sidecar 排程核心、CNC/整形 CNC/特殊大件/缺口诊断算法。
...
| 文件名疑似错误规则：正常 565/测试片用例不误报 | 通过 ✓ |
| 文件名疑似错误规则：`安容克送` / `安容克送检` / `安容克送样` / `安容克上传` 预填送测人并提示确认 | 通过 ✓ |
| 文件名疑似错误规则：`25点70` 提示送测时间异常 | 通过 ✓ |
| 文件名疑似错误规则：多个 PCS / `-手量-送` 提示人工确认 | 通过 ✓ |
| 文件名疑似错误规则：EVT-SLIDER 无品名提示人工确认 | 通过 ✓ |
| 文件名疑似错误规则：`首件` 不再误报为数量缺失 | 通过 ✓ |
| ManualTaskDialog 显示识别 warning | 完成 ✓ |
| HelpCenterDialog 同步疑似错误边界 | 完成 ✓ |
| `npx.cmd tsc --noEmit`（疑似错误规则后） | 通过 ✓ |
| `cargo check --release`（疑似错误规则后） | 通过 ✓ |
| `python sidecar\build_sidecar.py`（疑似错误规则后） | 成功 ✓ |
| `npm.cmd run tauri build`（疑似错误规则后） | 成功 ✓，仅 Vite chunk size 警告 |
| `scripts/package-portable.ps1 -Version 5.0.7`（疑似错误规则后） | 成功 ✓ |
...
来源：`releases\OMM日报系统_便携版_5.0.7\manifest.json`，`packaged_at=2026-06-30T20:36:07`。

```text
[app] OMM日报系统.exe
sha256=97c78b104f470fde5e1b24ba02f8dd66c6f56edb3be9a7e4c17d58177877ecc0
...
9. 对文件名疑似错误提示做真实 GUI 验收：在手量弹窗粘贴 `安容克送`、`25点70送测`、`96PCS-12PCS`、`-手量-送` 等例子，确认界面显示清楚的“需确认”原因。
...
17. 帮助文档已同步每件时间偏低提示。
18. 文件名疑似错误/需确认规则已精确化：品名异常、多候选、送测人疑似错字、送测时间异常、手量测量员异常、工站兜底等都会生成可解释 warning。
19. 手量候选卡片、粘贴识别日志、识别补充测试窗口会显示这些 warning；正常 565/测试片用例不误报。
20. 版本号仍保持 5.0.7。
...
最新便携版 manifest（packaged_at=2026-06-30T20:36:07）：
- app: 97c78b104f470fde5e1b24ba02f8dd66c6f56edb3be9a7e4c17d58177877ecc0
...
建议下一轮工作：
1. 真实 GUI 验收（A-E + 手量识别/耗时输入 + 每件时间偏低提示 + 文件名疑似错误 warning）。
