...
本轮目标：
只做最小必要改进。当前 A-E、手量识别细化、手量耗时输入体验已完成。后续优先做 GUI 验收、真实手量持久化方案设计、或用户明确的新需求。不要大规模重构，不要触碰 sidecar 排程核心。
...
6. （已完成）细化手量文件夹识别：品名优先数字/料号；第二段识别为工站；支持“姓名送测”；测量员只从“-手量-姓名”识别。
7. （已完成）手量耗时输入体验：不再从文件夹名识别耗时；默认按小时输入（2=120分钟）；实时显示换算结果；未识别不列为强警告。
8. 真实手量持久化先不要直接实现；如要做，先写方案，原则是不写回原始日期目录，而是写配置目录的独立 json。
9. 完成后更新帮助文档和 opencode-handoff-v5.0.6.md。
...
6. 手量文件夹 `565-开发-MO-T0模具测试-安容克送测-96PCS-CMM-郑安午-手量-禹欣`：品名=565，工站=开发，送测人=安容克，测量员=禹欣，耗时待填写。
7. 手量弹窗耗时输入：`2`→`将按 120 分钟计入`；`2.5`→`150 分钟`；`150分钟`→`150 分钟`。
8. 手量耗时未填写时，候选信息不显示橙色强警告，保存/生成时提示“请填写手量耗时”。
